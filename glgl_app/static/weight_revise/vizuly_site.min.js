...
function loadData(){
d3.csv(
    "/static/weight_revise/weightedtree_federal_budget.csv",
    function(a){

        data.values=prepData(a),
        initialize()
...
function prepData(a){
    ...
    if(a&&(a.id=c+"_"+d,a.values))

    for(var e=a.values.length-1;e>=0;e--)
        a.id=c+"_"+e,
        a.values[e].key||a.values[e].Level4?b(a.values[e],a.id,e):a.values.splice(e,1)
    }
    var c=[];

    a.forEach(function(a,b){for(var d=0,b=0;b<valueFields.length;b++)d+=Number(a[valueFields[b]]);d>0&&c.push(a)});

    var d=d3.nest()
             .key(function(a){return a.Level1})
             .key(function(a){return a.Level2})
             .key(function(a){return a.Level3})
             .entries(c);
    vizuly.data.aggregateNest(d,valueFields,function(a,b){return Number(a)+Number(b)});

    var e={};

    return e.values=d,b(e,"0","0"),d

}

function initialize(){viz=vizuly.viz.weighted_tree(document.getElementById("viz_container")),theme=vizuly.theme.weighted_tree(viz).skin(vizuly.skin.WEIGHTED_TREE_AXIIS),viz.data(data).width(600).height(600).children(function(a){return a.values}).key(function(a){return a.id}).value(function(a){return Number(a["agg_"+valueField])}).fixedSpan(-1).label(function(a){return trimLabel(a.key||a["Level"+a.depth])}).on("measure",onMeasure).on("mouseover",onMouseOver).on("mouseout",onMouseOut).on("click",onClick),changeSize(d3.select("#currentDisplay").attr("item_value")),viz.toggleNode(data.values[2]),viz.toggleNode(data.values[2].values[0]),viz.toggleNode(data.values[3])}

function trimLabel(a){return String(a).length>20?String(a).substr(0,17)+"...":a}

function createDataTip(a,b,c,d,e){var f=datatip.replace("HEADER1",c);f=f.replace("HEADER2",d),f=f.replace("HEADER3",e),d3.select("body").append("div").attr("class","vz-weighted_tree-tip").style("position","absolute").style("top",b+"px").style("left",a-125+"px").style("opacity",0).html(f).transition().style("opacity",1)}

function onMeasure(){}

function onMouseOver(a,b,c){if(console.log("mouse over"),b!=data){var d=a.getBoundingClientRect();b.target&&(b=b.target),createDataTip(d.left,d.top+.05*viz.height(),b.key||b["Level"+b.depth],formatCurrency(b["agg_"+valueField]),valueField)}}

function onMouseOut(a,b,c){d3.selectAll(".vz-weighted_tree-tip").remove()}

function onClick(a,b,c){viz.toggleNode(b)}

function changeSkin(a){"None"==a?theme.release():(theme.viz(viz),theme.skin(a)),viz().update()}

function changeSize(a){var b=String(a).split(",");viz_container.transition().duration(300).style("width",b[0]+"px").style("height",b[1]+"px"),viz.width(b[0]).height(.8*b[1]).update()}

function changeData(a){valueField=valueFields[Number(a)],viz.update()}
