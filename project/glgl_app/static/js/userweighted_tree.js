...
function loadData() {
    ...
    $.get('/showallpath/', function(root) {//d3.json这个函数在d3.js里面
    ...
    function recurse(node) {//这里只是定义了函数，但是未被调用，在后面才被调用  reduce回调函数
        ...
        if (node.children){
            for (x in node.children){
                temp2 = recurse(node.children[x])+weights;
                weights = temp2;
            };
        }
        if (!node.id) node.id = ++i;
        temp1 = parseFloat(node.weight)+weights;
        node.weight = temp1;
        return parseFloat(node.weight);
    }

    temp = recurse(root);//调用前面定义的函数，存入参数root，root才之前被赋予了JSON数据
    return root;
}
*/
function flatten(root) {
...
function trimLabel(label) {
   return (String(label).length > 12) ? String(label).substr(0, 9) + "..." : label;
}
...
function onMouseOver(e,d,i) {
    ...
    createDataTip(rect.left, rect.top, d.key ,"掌握程度", (d["achieve"]));
...
function click(d) {
    ...
    document.getElementById("videos").value = d.__data__.videos;
    document.getElementById("knowledge_name").value = d.__data__.key;
